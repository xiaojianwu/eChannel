...
pj_status_t handleEvents(unsigned maxMsec,unsigned *pCnt)
{
	...
	int c = pj_timer_heap_poll(iceCfg.ice_cfg.stun_cfg.timer_heap,&timeOut) ;
	if (c>0) {count+=c;}
    ...
    */
	...
	do 
	{
		c=pj_ioqueue_poll(iceCfg.ice_cfg.stun_cfg.ioqueue,&timeOut);
		if (c<0){
...
bool iceAppInit(const char *rfc5766  ) 
{
	...
	pj_status_t status ;	
	iceCfg.opt.comp_cnt=1; 
	iceCfg.opt.max_host=-1;
	iceCfg.opt.regular =PJ_FALSE;
	iceCfg.opt.turn_tcp = PJ_TRUE;
	iceCfg.opt.stun_srv=pj_str((char*)rfc5766);
	iceCfg.opt.turn_srv=pj_str((char*)rfc5766);
	//iceCfg.opt.turn_username=pj_str("test");
	//iceCfg.opt.turn_password=pj_str("test");
	iceCfg.opt.turn_fingerprint=PJ_TRUE ;
	do 
	{	
		/* Must create pool factory, where memory allocations come from */
		pj_caching_pool_init(&iceCfg.cp, NULL, 0);
		/* Init our ICE settings with null values */
		pj_ice_strans_cfg_default(&iceCfg.ice_cfg);
		iceCfg.ice_cfg.stun_cfg.pf = &iceCfg.cp.factory ;
		/* Create application memory pool */
		iceCfg.pool = pj_pool_create(&iceCfg.cp.factory, "iceApp",512, 512, NULL);
		/* Create timer heap for timer stuff */
		status = pj_timer_heap_create(iceCfg.pool,100,&iceCfg.ice_cfg.stun_cfg.timer_heap);
		CHCKE_BREAK("pj_timer_heap_create",status) ;			

		/* and create ioqueue for network I/O stuff */
		status = pj_ioqueue_create(iceCfg.pool, 16, &iceCfg.ice_cfg.stun_cfg.ioqueue) ;
		...
		*/
		...
		iceCfg.ice_cfg.af = pj_AF_INET();

		 //******************************dns does't need*******************************
		 /* -= Start initializing ICE stream transport config =- */

		 /* Maximum number of host candidates */
		 if (iceCfg.opt.max_host != -1)
			 iceCfg.ice_cfg.stun.max_host_cands = iceCfg.opt.max_host;
		 /* Nomination strategy */
		 if (iceCfg.opt.regular)
			 iceCfg.ice_cfg.opt.aggressive = PJ_FALSE;
		 else
			 iceCfg.ice_cfg.opt.aggressive = PJ_TRUE;

		 /* Configure STUN/srflx candidate resolution */
		 if (iceCfg.opt.stun_srv.slen) {
			 char *pos;
			/* Command line option may contain port number */
			if ((pos=pj_strchr(&iceCfg.opt.stun_srv, ':')) != NULL) {
				iceCfg.ice_cfg.stun.server.ptr = iceCfg.opt.stun_srv.ptr;
				iceCfg.ice_cfg.stun.server.slen = (pos - iceCfg.opt.stun_srv.ptr);

				iceCfg.ice_cfg.stun.port = (pj_uint16_t)atoi(pos+1);
			} else {
				iceCfg.ice_cfg.stun.server = iceCfg.opt.stun_srv;
				iceCfg.ice_cfg.stun.port = PJ_STUN_PORT;
			}

			/* For this demo app, configure longer STUN keep-alive time
			 * so that it does't clutter the screen output.
			 */
			iceCfg.ice_cfg.stun.cfg.ka_interval = KA_INTERVAL;
		}

		/* Configure TURN candidate */
		if (iceCfg.opt.turn_srv.slen) {
		char *pos;

		/* Command line option may contain port number */
		if ((pos=pj_strchr(&iceCfg.opt.turn_srv, ':')) != NULL) {
			iceCfg.ice_cfg.turn.server.ptr = iceCfg.opt.turn_srv.ptr;
			iceCfg.ice_cfg.turn.server.slen = (pos - iceCfg.opt.turn_srv.ptr);

			iceCfg.ice_cfg.turn.port = (pj_uint16_t)atoi(pos+1);
			} else {
				iceCfg.ice_cfg.turn.server = iceCfg.opt.turn_srv;
				iceCfg.ice_cfg.turn.port = PJ_STUN_PORT;
			}

			/* TURN credential */
			iceCfg.ice_cfg.turn.auth_cred.type = PJ_STUN_AUTH_CRED_STATIC;
			iceCfg.ice_cfg.turn.auth_cred.data.static_cred.username = pj_str(iceCfg.opt.turn_username);
			iceCfg.ice_cfg.turn.auth_cred.data.static_cred.data_type = PJ_STUN_PASSWD_HASHED;//PJ_STUN_PASSWD_PLAIN;
			iceCfg.ice_cfg.turn.auth_cred.data.static_cred.realm=pj_str(iceCfg.opt.turn_reaml );/*pj_str("www.vemic.com");*/
			iceCfg.ice_cfg.turn.auth_cred.data.static_cred.data = pj_str(iceCfg.opt.turn_password);

			/* Connection type to TURN server */
			if (iceCfg.opt.turn_tcp)
				iceCfg.ice_cfg.turn.conn_type = PJ_TURN_TP_TCP;
			else
				iceCfg.ice_cfg.turn.conn_type = PJ_TURN_TP_UDP;

			/* For this demo app, configure longer keep-alive time
			 * so that it does't clutter the screen output.
			 */
			iceCfg.ice_cfg.turn.alloc_param.ka_interval = KA_INTERVAL;
		}
	} while (false);
	return (status==PJ_SUCCESS) ;
}
...
void cbOnRxData(pj_ice_strans *iceSt,unsigned compId,void *pkt,pj_size_t size,
	...
	if (appNet!=NULL ) {
		appNet->dataRead(pkt,size,compId) ;
	}else{
	...
	if(pUser !=NULL && PJ_ICE_STRANS_OP_INIT==op ){
		...
		for(int i=1;i<=compCnt;++i){
			...
			if(vlist!=NULL){
				...
				print_error_log("cbOnIceComplete negotiation completed remote addr=%s type=%s,local addr=%s,type=%s",
...
void createSession(void * pUser,const char*Name ,const char* rfc5766 ){		
	do 
	{
		if (pUser == NULL){
		...
		status = pj_ice_strans_create(Name,		/* object name  */
			&iceCfg.ice_cfg,	    /* settings	    */
			iceCfg.opt.comp_cnt,  /* comp_cnt	    */ //Yongwenz 可以修改的地方
...
static void destoryIceApp(pj_ice_strans *icest )
{
	if (--iceCfg.ref==0){
		...
		if(iceCfg.ice_cfg.stun_cfg.ioqueue){
			pj_ioqueue_destroy(iceCfg.ice_cfg.stun_cfg.ioqueue) ;
		}
		if(iceCfg.ice_cfg.stun_cfg.timer_heap){
			pj_timer_heap_destroy(iceCfg.ice_cfg.stun_cfg.timer_heap) ;
		}
...
int encodeSession(char buffer[], unsigned maxlen,ICE_Session *pSession)
{
    ...
    for (comp=0; comp<iceCfg.opt.comp_cnt; ++comp) {
	unsigned j, cand_cnt;
